...
due to challenges such as determining homology of molecular
...
Despite these difficulties, considerable bioinformatic advances 
...
With over 5000 DNA sequences available in GenBank from previous studies, 
...
and discuss the novel method used here to construct the supermatrix.
...
fossil constraints on genus Epilobium, it may be that the fossil described should
have constrained Epilobium + Chamerion due to taxonomic changes.....
need to check fossil synapomorphies to see where it should be placed!

Petal color and petal number were found to follow a pattern 
...
Further analyses should be performed to test 
whether shifts in floral traits are correlated with pollinator shifts, 
and to determine how these shifts affect diversification rates. 
Furthermore, this work should be extended to test historical 
...
However, the method used by SUMAC combines both approaches;
...
homologous clusters in a more targeted manner. SUMAC provides
a framework with which GenBank can be repeatedly mined 
using different taxonomic groups and/or different guide sequences
and the resulting data matrices can be compared.

describe some recently developed metrics that can
